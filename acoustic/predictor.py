...
class EnergyPredictor(torch.nn.Module):
    ...
    def inference(self, xs: torch.Tensor, olens=None, alpha: float = 1.0):
        ...
        Returns:
            LongTensor: Batch of predicted durations in linear domain (B, Tmax).
        """
        f0_spec, f0_mean, f0_std = self.forward(
            xs, olens, x_masks=None
        )  # (B, Tmax, 10)
        # print(f0_spec)
...
class PitchPredictor(torch.nn.Module):
    ...
    def inference(self, xs: torch.Tensor, olens=None, alpha: float = 1.0):
        ...
        f0_reconstructed = f0_reconstructed * alpha
    ...
    def inverse(self, Wavelet_lf0, f0_mean, f0_std):
        ...
        f0_reconstructed = (
            torch.Tensor(lf0_rec_sum_norm).to(device=device) * f0_std
        ) + f0_mean
