...
class AudioPreprocessor:
    ...
    def __init__(self, input_sr, output_sr=None, melspec_buckets=80, hop_length=256, n_fft=1024, cut_silence=False, device="cpu", fmax_for_spec=8000):
        ...
        # self.temp_resample = Resample(orig_freq=22050, new_freq=16000).to(self.device)
        # self.temp_resample_rev = Resample(orig_freq=16000, new_freq=22050).to(self.device)
    ...
    def cut_silence_from_audio(self, audio):
        ...
        with torch.inference_mode():
            # audio = self.temp_resample(audio)
            speech_timestamps = self.get_speech_timestamps(audio, self.silero_model, sampling_rate=16000)
            # audio = self.temp_resample_rev(audio)
        try:
    ...
    def logmelfilterbank(self, audio, sampling_rate, fmin=40, fmax=None, eps=1e-10):
        ...
        return torch.log(torch.Tensor(np.dot(spc, mel_basis.T))).transpose(0, 1)
...
if __name__ == '__main__':
    ...
    ap = AudioPreprocessor(input_sr=sr, output_sr=22050, cut_silence=True)
